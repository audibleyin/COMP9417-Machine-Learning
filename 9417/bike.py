...
for name in ['train', 'test']:
    df = pd.read_csv('F:/9417/bike-sharing-demand/%s.csv' % name)
    df['_data'] = name
    dfs[name] = df
...
def prep_data(data, input_cols):
    ...
    y_r = data['registered_log'].to_numpy()
    y_c = data['casual_log'].to_numpy()
...
def predict_on_test_set(model, x_cols):
    ...
    y_train_cas = df_train['casual_log'].to_numpy()
    y_train_reg = df_train['registered_log'].to_numpy()
